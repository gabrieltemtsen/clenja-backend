...
import { CreateOrgDto, UpdateOrgDto, CreateInviteDto, UpdateMemberDto } from './dto/orgs.dto';
...
export class OrgsService {
     ...
     */
    async createOrg(userId: string, dto: CreateOrgDto): Promise<{ org: Org; wallet: Wallet }> {
    ...
    async updateOrg(orgId: string, userId: string, dto: UpdateOrgDto): Promise<Org> {
        await this.requireRole(orgId, userId, [OrgMemberRole.OWNER, OrgMemberRole.ADMIN]);
    ...
    async getUserInvites(userId: string, email?: string): Promise<OrgInvite[]> {
        const conditions: any[] = [{ inviteeUserId: userId, status: OrgInviteStatus.PENDING }];
    ...
    async updateMember(
        ...
        if (actor.role === OrgMemberRole.ADMIN && dto.role === OrgMemberRole.OWNER) {
     ...
     * Remove a member.
     */
    async removeMember(orgId: string, memberId: string, actorUserId: string): Promise<void> {
        await this.requireRole(orgId, actorUserId, [OrgMemberRole.OWNER, OrgMemberRole.ADMIN]);
