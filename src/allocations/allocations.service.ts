...
export class AllocationsService {
    ...
    async updateAllocation(
        ...
        if (dto.managerMemberId) {
            ...
            const manager = await this.memberRepo.findOne({
                where: { id: dto.managerMemberId, orgId: allocation.orgId, status: OrgMemberStatus.ACTIVE },
            });
            if (!manager) {
                throw new BadRequestException('Manager member not found');
            }
    ...
    async fundFromOrg(
        ...
        this.logger.log(`Allocation ${allocationId} funded with ${dto.amount} kobo`);
        return {
            success: true,
    ...
    async fundFromParent(
        ...
        await this.ledgerService.postTransaction({
            ...
            destinationWalletId: allocation.walletId,
            description: dto.description || `Sub-allocation funding: ${allocation.name}`,
        });
