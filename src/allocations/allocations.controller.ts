...
export class AllocationsController {
  ...
  async createAllocation(
    ...
    const { allocation, wallet } =
      await this.allocationsService.createAllocation(orgId, userId, dto);
    return {
      success: true,
      data: {
        id: allocation.id,
  ...
  async getOrgAllocations(@Param('orgId') orgId: string) {
    ...
    const data = await Promise.all(
      allocations.map(async (allocation) => {
        const wallet = await this.allocationsService.getAllocationWallet(
          allocation.id,
        );
        return {
          ...allocation,
  ...
  async getAllocation(@Param('allocationId') allocationId: string) {
    const allocation =
      await this.allocationsService.getAllocation(allocationId);
    const wallet =
      await this.allocationsService.getAllocationWallet(allocationId);
    return {
      success: true,
      data: {
        ...allocation,
  ...
  async addRule(
    ...
    const rule = await this.allocationsService.addRule(
      allocationId,
      userId,
      dto,
    );
